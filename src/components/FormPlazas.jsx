...
import { deleteData, deleteRows } from "../redux/actions/plazas";
import Botones from "./Botones";
import { useEffect, useState } from "react";
import { DataTable } from "./DataTable";
 
const FormPlazas = (props) =>{
    const rows = useSelector((state)=>state.plazasReducer.rows);
    ...
    const obtenerData = async() =>{
        await axios.get('http://localhost:8080/api/plazas/all',
    ...
    const postData = async(input)=>{
        await axios.post(
            'http://localhost:8080/api/plazas/save', 
    ...
    return(
        ...
        <form id={props.name} className={s.formulario} onSubmit={handleSubmit(onSubmit)}>
            <div>
                ...
                type="text"
                {...register("idplaza", { required: true, minLength: 3 })}
                />
                <div>
