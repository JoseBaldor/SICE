...
import { deleteData, deleteRows } from "../redux/actions/areasAcademicas";
import Botones from "./Botones";
import { useEffect, useState } from "react";
import { DataTable } from "./DataTable";

const FormAcademicas = (props) =>{
    const rows = useSelector((state)=>state.areasAcademicasReducer.rows);
    ...
    const postData = async(input)=>{
        await axios.post(
            'http://localhost:8080/api/academicas/save', 
    ...
    return(
        ...
        <div className={s.table} > 
        {
            lista ? (
                <DataTable 
                titulo = {"Listado de Areas Académicas"}
                data = {data}
                columns = {columns}
