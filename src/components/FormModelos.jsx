...
import { deleteData, deleteRows } from "../redux/actions/plazasModelos";
import Botones from "./Botones";
import { useEffect, useState } from "react";
import { DataTable } from "./DataTable";

const FormModelos = (props) =>{
    const rows = useSelector((state)=>state.plazasModelosReducer.rows);
    ...
    const obtenerData = async() =>{
        await axios.get('http://localhost:8080/api/modelos/all',
    ...
    const postData = async(input)=>{
        await axios.post(
            'http://localhost:8080/api/modelos/save', 
    ...
    return(
        ...
        <form id={props.name} className={s.formulario} onSubmit={handleSubmit(onSubmit)}>
            <div>
                ...
                type="text"
                {...register("idplazamodelo", { required: true, minLength: 2 })}
                />
                <div>
                    {errors.idplazamodelo?.type === "required" && (
                        <small className={s.error}>La Clave no puede estar vacía...</small>
                    )}
                    {errors.idplazamodelo?.type === "minLength" && (
        ...
        <div className={s.table} > 
        {
            lista ? (
                <DataTable 
                titulo = {"Listado de Modelos de Plazas"}
                data = {data}
                columns = {columns}
