...
export const GET_UACADEMICAS = 'GET_UACADEMICAS';
...
export const deleteData = (id) =>{
    return async function(dispatch){
        try {
            const response = await axios.delete(`http://localhost:8080/api/unidades/delete/${id}`, 
            {
                headers: {
...
export const getUnidadesAcademicas = () =>{
    return async function(dispatch){
        await axios.get('http://localhost:8080/api/unidades/all',
            {headers:{"Content-Type" : "application/json"}})
            .then((response)=>{
                dispatch({type: GET_UACADEMICAS, payload: response.data})
        })
        .catch((error)=>{
